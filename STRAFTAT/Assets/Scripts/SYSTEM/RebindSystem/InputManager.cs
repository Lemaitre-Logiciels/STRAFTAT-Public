...
public class InputManager : MonoBehaviour
{
    ...
    {
        inputActions = new PlayerControls();
        foreach (var temp in rebindFields)
        {
            temp.Restart();
        }
    }
    ...
    public static void StartRebind(string actionName, int bindingIndex, TextMeshProUGUI statusText, bool excludeMouse, bool sequenceDisplay)
    {
        ...
        if (action.bindings[bindingIndex].isComposite)
        {
            ...
            if (firstPartIndex < action.bindings.Count && action.bindings[firstPartIndex].isPartOfComposite)
                DoRebind(action, firstPartIndex, statusText, true, excludeMouse, sequenceDisplay);
        }
        else
            DoRebind(action, bindingIndex, statusText, false, excludeMouse, sequenceDisplay);
    }
    ...
    private static void DoRebind(InputAction actionToRebind, int bindingIndex, TextMeshProUGUI statusText, bool allCompositeParts, bool excludeMouse, bool sequenceDisplay)
    {
        ...
        if (sequenceDisplay) PauseManager.Instance.sequenceDisplayGameObject.SetActive(true);

        statusText.text = $"Press a {actionToRebind.expectedControlType}";

        actionToRebind.Disable();

        var rebind = actionToRebind.PerformInteractiveRebinding(bindingIndex).WithCancelingThrough("<Keyboard>/escape");
        ...
        rebind.OnComplete(operation =>
        {
            ...
            if (allCompositeParts)
            {
                ...
                {
                    if (sequenceDisplay) PauseManager.Instance.sequenceDisplayGameObject.SetActive(true);
                    DoRebind(actionToRebind, nextBindingIndex, statusText, allCompositeParts, excludeMouse, sequenceDisplay);
                }
            }
    ...
    public static void ResetBinding(string actionName, int bindingIndex)
    {
        ...
        if (action.bindings[bindingIndex].isComposite)
        {
            for (int i = bindingIndex; i < action.bindings.Count && (action.bindings[i].isComposite || action.bindings[i].isPartOfComposite); i++)
            {
