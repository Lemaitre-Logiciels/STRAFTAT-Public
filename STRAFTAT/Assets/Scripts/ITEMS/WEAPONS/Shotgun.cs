...
public class Shotgun : Weapon
{
    ...
    private void Start()
    {
        hitPosition = new Vector3[bulletAmount];
    ...
    private void Update()
    {
        ...
        if (reload.ReadValue<float>() > 0.1f)
            if (reloadWeapon && chargedBullets != ammoCharge && !isReloading && fireTimer <= 0 && currentAmmo > 0 && IsOwner && !PauseManager.Instance.pause) StartCoroutine(Reload());
    ...
    private void Fire()
    {
        ...
        if (currentAmmo <=0 && chargedBullets <= 0) 
        {
            ...
            noAmmoClicks ++;
            if (noAmmoClicks > 2 && inRightHand) rootObject.GetComponent<PlayerPickup>().RightHandDrop();
            if (noAmmoClicks > 2 && inLeftHand) rootObject.GetComponent<PlayerPickup>().LeftHandDrop();
        }
        ...
        {
            hitPosition[i] = transform.right * Random.Range(spread, -spread) + transform.up * Random.Range(spread, -spread);
        }

        firstShot = true;
        destroyGlassOnce = false;
        
        for (int i=0; i < bulletAmount; i++)
        {
            ShootServer(damage, cam.transform.position, cam.transform.forward + hitPosition[i]);
        }
        ShootServerEffect();

        CameraRevolverAnimation();
        WeaponAnimation();
    ...
    private void ShootServer(float damageToGive, Vector3 position, Vector3 direction)
    {
        ...
        if (envHits.Length != 0)
        {
            for (int i = 0; i < envHits.Length; i++)
            {
                ...
                {
                    if (!destroyGlassOnce) BreakGlassServer(envHits[i].point, direction, envHits[i].transform.gameObject);
                    destroyGlassOnce = true;
                }
