...
public class Gun : Weapon
{
    ...
    {
        WeaponUpdate();
        if (gameObject.layer == 7) isBursting = false;
        ...
        if (!onePressShoot)
        {
            ...
            else if (fire2.ReadValue<float>() > 0.1f && inLeftHand && akAnim && currentAmmo > 0) 
                camAnimScript.rotateBack = false;
            else 
    ...
    private void Fire()
    {
        ...
        if ((burstGun || aimBurstGun) && isBursting) return;
        ...
        else if (aimBurstGun && playerController.isAiming) StartCoroutine(BurstFire());
        else {
            ...
            if (hit.transform.gameObject.layer == 11) {
                ...
                if (hit.transform.root.CompareTag("Player") ? hit.transform.root.TryGetComponent(out enemyHealth) : hit.transform.GetComponentInParent<PlayerHealth>() != null) {
                    ...
                    if (enemyHealth.health-damageToGive <= 0) {
                        LocalSound(2);
                        playerController.KillShockWave();
    ...
    {
        if (supp == null) return;
        if (supp.GetComponent<Suppression>() == null) return;
